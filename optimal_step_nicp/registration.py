...
def registration_mesh2pcl(template_mesh: o3d.geometry.TriangleMesh,
    ...
    template_faces = torch.tensor(np.asarray(
        template_mesh.triangles)).long().to(device).unsqueeze(0)

    # target vertices
    target_vertex = torch.tensor(np.asarray(
        target_pcl.points)).float().to(device).unsqueeze(0)
    ...
    boundary_mask = boundary_mask.unsqueeze(0).to(device).unsqueeze(2)
    ...
    for i in loop:
        ...
        if i % log_iter == 0:
            print(distance, stiffness_sum.item(), vert_sum.item(), laplacian_loss.item())
